...
extern std::vector<uint32_t> t_last_contact_update; // timestamp in milliseconds at last leg motor ground contact update
...
const int kLegAccelFilterLength = 6;        // number of leg acceleration samples to hold in the FIFO queue
...
const float kQddotLegContact = -1000;                   // leg acceleration in mm/s^2 above which (more negative) leg contact is likely
